	...
	public CancelTicket() {
		...
		contentPane.addMouseMotionListener(new MouseMotionAdapter() {
			...
			public void mouseDragged(MouseEvent arg0) {
				...
				CancelTicket.this.setLocation(x-xx,y-xy);
			}
		});
		...
		separator.setBounds(497, 190, 177, 13);
		contentPane.add(separator);
		
		JSeparator separator_1 = new JSeparator();
		separator_1.setPreferredSize(new Dimension(0, 3));
		separator_1.setForeground(SystemColor.textHighlight);
		separator_1.setBounds(69, 190, 199, 13);
		...
		JButton btnViewTicket = new JButton("CANCEL TICKET");
		btnViewTicket.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				st1.setString(1,  pnrNo.getText());
				
				ResultSet rs=st.executeQuery();
				ResultSet rs1=st1.executeQuery();
					if(rs1.next())
					{
						JOptionPane.showMessageDialog(null,"This Ticket is already Cancelled");
					}
					else if(rs.next()) {
						JFrame frame1 = new JFrame("");
						if(JOptionPane.showConfirmDialog(frame1, "Confirm if you really want to Cancel Ticket of Pnr No."+pnr, "Ticketing System", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_NO_OPTION)
						{
							
							String sql="UPDATE booking set Status=? WHERE PnrNo=?";
							PreparedStatement p=con.prepareStatement(sql);
							p.setString(1, "Cancelled");
							p.setString(2, pnr);
							int updateCount=p.executeUpdate();
							String sql1="UPDATE booking set SeatNo=? WHERE PnrNo=?";
							PreparedStatement p1=con.prepareStatement(sql1);
							p1.setString(1, "-");
							p1.setString(2, pnr);
							int updateCount1=p1.executeUpdate();
							
							
							JOptionPane.showMessageDialog(null,"SUCESSFULY Cancelled Ticket of Pnr No"+pnr);
		...
		label_2.setBounds(280, 152, 62, 66);
		gradientPanel.add(label_2);
		label_2.setIcon(new ImageIcon("C:\\Users\\Mandy\\Desktop\\Mandar\\workspace-Mandar\\railway management system\\IMAGES\\Delete Ticket_50px_1.png"));
		
		JLabel lblNewLabel_3 = new JLabel("Cancel Ticket");
		lblNewLabel_3.setBounds(341, 183, 147, 22);
